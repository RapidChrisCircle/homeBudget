...
- **Stat tiles** (`widget_type: stat_tile`) — one headline figure over a period: Total Income, Total Expenses, Avg Per Month, Avg Per Transaction, Net Saved, or a plain transaction count, each reading `GET /api/reports/kpis` (`services/dashboard_metrics.dashboard_kpis`, built on `reporting.category_grid`/`trends.monthly_summaries` for its totals, not a second independent aggregation). Avg Per Month and Avg Per Transaction are both **spend-rate** figures — total expenses divided by the window's months, or by its count of expense transactions (counted `DISTINCT` by transaction, so a split grocery run counts once, not once per allocation) — deliberately not mixed with income, which would answer neither "how much do we spend a month" nor "how much do we earn". A tile whose metric maps to a real kind (income/expense) is clickable, opening the ledger filtered to that kind across the whole window; Net Saved and the plain count aren't, since neither is a transaction kind. Rendered without `<Amount>`'s tabular numerals — a large standalone figure reads better in the font's normal proportional numerals, reserving tabular alignment for columns of numbers that must line up.
