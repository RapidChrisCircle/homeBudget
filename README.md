...
**Intermittent version**, where errors move around on reload instead of every call failing the same way — one page's cards load fine while another's says "Not Found", and reloading changes which: this is the same root cause (an application that wasn't created cleanly), but with **two** `api` containers left running instead of a doubled name, both answering the `api` DNS name Docker load-balances across. A request landing on the stale one 404s; the next request, seconds later, lands on the correct one and works — which is also why `GET /api/version` in the footer can show the right version at the very same moment another card fails, and why the failing page is never the same twice.

To confirm before deleting anything:
- Open a container's shell (Container Station → the `web` container → Console, or `docker exec`) and run `getent hosts api`. **More than one address is the smoking gun** — one address means this isn't the cause.
- Every response, success or failure, now carries `X-App-Version`/`X-App-Commit` headers (`backend/app/main.py`'s `add_build_identity_headers`, sourced from the same `app/version.py` `GET /api/version` reports) specifically so this is checkable from outside a shell too: reload a failing page a few times with the browser's Network tab open and compare the header across requests — a different value (or the header vanishing) between otherwise-identical calls means two builds are answering. The frontend performs this same comparison itself and shows *"Responses are coming from more than one API build"* in the footer the first time it notices, without needing devtools at all.
- Error messages are also no longer bare "Not Found" — `frontend/src/services/api.ts`'s response interceptor enriches an ambiguous error (no `detail`, or FastAPI's own generic route-matching body) with the method, path and status that actually failed, e.g. `404 Not Found — GET /api/transactions did not match any route on the API that answered`. A real, specific error from one of this app's own endpoints (`"Category not found"`) is left completely untouched.

Once confirmed, the remedy is the same as the doubled-name case: delete the application and recreate it from scratch with application name `homebudget` rather than stopping containers one at a time — a partial cleanup is how the duplicate got there in the first place.
