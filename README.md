...
| Page | What it does |
|---|---|
| `/` | Dashboard — a customisable grid of widgets (account balances, net worth, cash flow and net worth charts, KPI stat tiles, a transaction calendar, a spending-pace sparkline, this month's totals, over-budget categories, goals summary, uncategorized count, recent activity); add, remove, resize and reorder via **Edit dashboard**/**Add new widget** |
| `/transactions` | Import CSVs, add transactions by hand, and the filterable, paginated ledger |
...
- **Command palette** (`Ctrl+K`/`Cmd+K`, `frontend/src/components/CommandPalette.jsx`) — the one UI element in the app that exists purely for keyboard users, mounted once in `App.jsx` outside the routed area so it survives navigation. Searches pages (`pageRegistry.jsx`), categories, accounts and merchants, grouped by kind, with arrow keys to move the selection and Enter to activate. Categories and accounts are fetched once, lazily, the first time the palette is opened — not on every page load, which would add two requests to every view for a feature most sessions never open — while merchants are searched live and debounced (`GET /transactions/groups?search=`, the same endpoint the ledger's own Group by merchant view uses), since there is no "list every merchant" endpoint to fetch upfront. Escape or clicking outside closes it.
- **Grouped navigation** (`pageRegistry.jsx`'s `group` field, `PAGE_GROUP_ORDER`) — nine-plus destinations in one flat row forced a scan every time. Each page declares a `group` (*Money*, *Plan*, *Insight*, *Setup*); a page with none (Home, Alerts) renders as its own standalone link instead, the same treatment Home has always had, since both are cross-cutting rather than a content area. Below 900px (the same breakpoint the widget grid already collapses at) the nav stacks into full-width rows instead of wrapping into one dense block.
- **Responsive ledger tables** (`.responsive-table` in `App.css`, `TransactionsPage.jsx`) — below 700px, the ledger's plain and grouped-by-merchant tables both become one card per row instead of forcing horizontal scroll: the `<thead>` is visually hidden (clipped, not `display:none`, so a screen reader in table-navigation mode still gets real column headers) and each `<td>` carries its own `data-label`, restored as a `::before` label once the table's own header row is gone. A `<td>` with no label (a row checkbox, the Details toggle) is self-explanatory without one.
- **Shared chart tooltip** (`components/charts/ChartTooltip.jsx`) — hover used to be a native SVG `<title>` only: slow to appear, unstyled, and it never fires on keyboard focus at all. `<title>` is left completely in place (nothing about a chart's existing accessible-name contract changes); a themed HTML tooltip is layered on top, positioned from the hovered point/bar's own `getBoundingClientRect()` rather than SVG viewBox math (which would misplace it on any chart not rendered at exactly 1:1). It fires on `mouseenter`/`mouseleave` for every point/bar, and additionally on `focus`/`blur` for whichever ones are already keyboard-focusable — the clickable, drill-down-enabled points/bars — so a keyboard user gets the identical tooltip a mouse user does, without turning every decorative dot on an un-drillable chart into an extra Tab stop.
- **Toast notifications** (`services/toast.ts`, `components/ToastContainer.jsx`) — a transient, bottom-right confirmation ("Imported 5 transaction(s)", "Budget saved") for an action whose success might otherwise sit in text a user has already scrolled past. State lives in a module-level pub-sub store outside React (the same pattern `services/api.ts`'s own build-mismatch detection already uses), so any call site can fire one via `showToast()` without needing to be inside whatever component renders `<ToastContainer>` (mounted once in `App.jsx`). Scoped to success confirmations only — `<ErrorState>` remains the one place any error appears, which is what keeps this from ever becoming "the only place an error shows". Respects `prefers-reduced-motion` (the fade/slide-in animation is dropped, same as every other animation in the app). Wired into a representative set of call sites (import, budget save, preset apply, rule apply) rather than an exhaustive retrofit of every mutation — the infrastructure is reusable, so adding a toast to a new call site later is a one-line addition.
...
A row whose narration starts with `AUTHORISATION ONLY - ` **and** carries a numeric Debit is skipped entirely, not imported — a bank's placeholder for a card pre-authorisation hold that later settles as its own row, so importing both would double-count the spend (`services/csv_import._is_pending_authorisation`). The narration prefix alone isn't enough to skip a row — a bank can label a settled refund the same way, and that one carries real money that must not vanish — so the Debit check is what tells a genuine hold apart from ordinary text reuse. Skipped before that row's own fields are validated, so a hold with a blank Balance (some exports don't bother populating one for a pending hold) can't reject the whole file. Applies to **future imports only**; nothing already in the ledger is touched. The import result and the batch history table both show a `skipped_authorisation_count` alongside the existing skipped-duplicates figure, and the mapping preview panel reports its own (preview-scoped) count before anything is committed.
...
Import is not the only way money gets into the ledger. **Add a Transaction** (`/transactions`, `POST /api/transactions`) records a row by hand — cash spending, a reimbursement, or anything else that hasn't (or never will) come from a bank export. Pair it with a dedicated account created the normal way (`POST /accounts`, any account number you choose — e.g. `CASH`) if you want somewhere for it that isn't a real bank account.

Unlike an imported row, there is no bank-reported balance to trust. `Transaction.balance` is instead **computed**: the account's own current balance (the same "most recent by date, then id" resolution used everywhere else in the app — see [Balances and reports](#balances-and-reports)) plus this row's own signed amount. That computation is only meaningful if the new row *becomes* the account's latest transaction, so **a manual entry must be dated on or after the account's current latest transaction** — it can extend the ledger forward, never be inserted into the middle of a real bank history. Inserting one earlier would leave every later imported row's own bank-reported balance silently inconsistent with what actually happened, with no way to fix it short of recomputing balances the bank never gave us in the first place. The form's own **Money out / Money in** choice plus a positive amount (not a signed one) is translated into the app's usual debit-negative/credit-positive convention at the API boundary, so entering a $45 coffee never means typing `-45`.

A manual row with no category chosen is auto-categorized by a matching rule immediately, the same as an imported one — no need to wait for **Apply rules now**. Only a manual row's own fields (date, narration, amount, category, note, type) can be edited after creation, via the same **Edit** affordance an imported row's Details disclosure does *not* offer — an imported row is the bank's own record and stays editable only through category/note/splits, exactly as before manual entry existed (`Transaction.is_manual`). Editing re-runs the same "must still be the latest" check against every *other* transaction on the account. Deleting a manual transaction works exactly like deleting any other row — no special case.

Every manual entry shares one `ImportBatch` (filename **"Manually added"**), reused across entries rather than creating a fresh one-row batch each time, so frequent cash entry doesn't flood the Import card's batch history. Deleting that batch (from the same history table any import appears in) removes every manual entry at once — a real, visible action, not a hidden side effect.

One caveat worth knowing: if a bank statement later arrives covering the same period as a hand-entered transaction, import's duplicate detection (same account, date, narration, debit, credit **and balance**) will not recognize it as the same transaction — a manual row's balance is computed, not the bank's own figure, so the keys can't match. The statement import will add its own row alongside the manual one; reconciling the two (deleting whichever is now redundant) is a manual step.

## Exporting

Two genuinely different jobs (`services/export.py`), not one export with a format flag:

- **Export CSV**, on the ledger's own filter bar (`GET /api/transactions/export`) — exactly the rows the ledger currently shows for the active filters, sort included, with **no pagination**: an export is precisely the case where "everything I filtered to" must not be cut down to one page. It shares its filter validation and query-building with `GET /transactions` itself (`api/transactions._validated_ledger_query`), so the two can never accept a filter combination differently. Categories are resolved to names, and a **split** transaction's several allocations are flattened into one cell (`Groceries: -30.00; Alcohol: -20.00`) rather than either picking one arbitrarily or adding a second CSV row per allocation, which would silently break "one row per transaction" for anything reading the file downstream. **This is not designed to round-trip back through import** — its header doesn't match the built-in bank layout (`services/csv_formats.py` matches a layout by *exact* header string), and it carries Category/Note/split information no bank export format has a column for. That is a deliberate trade: a categorized, split-aware CSV is what makes it useful outside the app at all — a re-importable bank-shaped export would be a second, more limited file nobody asked for.
- **Download backup (JSON)**, on `/accounts` (`GET /api/export/database`) — every table, unfiltered and unpaginated (archived categories and goals included — a partial backup is a false sense of security), built from the *same* response schemas the API's own list endpoints already return, so a record in the snapshot can never describe itself differently than fetching it from the app would. There is deliberately **no restore-from-snapshot endpoint**: recreating a household's data by replaying each entity's own `POST` endpoint by hand is possible, but true point-in-time recovery is a Postgres-level restore, which this app has no business reimplementing.

Both are plain download links (`<a href="/api/...">`), not calls through the app's own `api` client — the browser saves the file itself from the response's `Content-Disposition` header, with no blob-URL machinery needed on the frontend.
...
Every filter lives in **one toolbar above the table**, as a row of chips — Date, Account, Narration, Amount, Category, Type — each opening its own popover with **Apply**/**Clear** inside it. Nothing filters per keystroke, only on Apply; Esc or clicking away closes without applying. An active chip **shows its value** (`Account: Joint Everyday`, `Date: 01/07/26 – 31/07/26`), so the whole row states what the view is narrowed to without opening anything, and **Clear all filters** carries the count of filters in force (a date or amount *range* counts once, not twice) and greys out when there are none. **Export CSV**, alongside it, downloads exactly this filtered set — see [Exporting](#exporting).

The toolbar is deliberately **identical whether or not Group by merchant is on**. Filters used to live on the column headers they narrow, Excel-style — which worked until the grouped view replaced Date/Account/Narration/Amount/Category with Merchant/Count/Total and left those filters nowhere to live, so they were duplicated as an inline copy that appeared only while grouped. The toolbar rearranging itself around the reader was the cost, and one home for filtering is the fix; the table headers keep **sorting**, which is genuinely per-column and has no equivalent problem.

| Filter | What it carries |
|---|---|
...
| Category | exact match, and also matches a **split** transaction with an allocation in that category (see Splits, below); **Uncategorized only** is a distinct mode from "all categories" and excludes split transactions entirely — a split row is never uncategorized even though its own `category_id` is null. The same dropdown also carries **Income**/**Expenses**/**Transfers** — every transaction of that *kind* regardless of which specific category, matching a split transaction the same split-aware way (`kind`, `services/ledger.py`'s `TransactionFilters`) — this is what every chart's drill-down (see [Trends](#trends) and [Dashboard](#dashboard)) opens the ledger to, and it's a real, clearable filter here too, not just a URL param a chart can reach but the UI can't |
| Type | Transaction type, case-insensitive exact match. It has no column of its own (it lives behind each row's Details disclosure, below), which used to make it the one filter rendered as a bare select among the popovers — being column-less stopped meaning anything once the chips stopped being tied to columns |

A **totals strip** sits between the toolbar and the table — money in, money out and net for the *whole filtered set*, not just the rows on the current page (`services/ledger.ledger_totals`, the same "count the whole thing in SQL, not what's on screen" rule sorting and the page-count already follow). It sums each matching row's own signed amount, exactly what that row already shows via its own `<Amount>` — a split transaction matched through only one of its several allocations still contributes its **full** amount here, the same as it already does as one row in the table, not the narrower per-category slice `/reports` would use instead. Identical above both the plain and the grouped-by-merchant table, since grouping only changes how the same filtered set is displayed, not what it totals to (`services/ledger.transaction_group_totals` is defined to always agree with the plain view for the same filters).

Contradictory filter combinations are rejected with a 422 rather than quietly returning nothing — `uncategorized` with a `category_id`, `account_id` with `account_group_id`, an inverted date range, or an inverted amount range.

Every column is sortable — click its label to cycle ascending → descending → off (`aria-sort` tracks the state). The ledger and the account-detail table (`/accounts/:id`) are paginated, so they sort **in SQL** (`services/ledger.py`'s `sort`/`direction` params, validated against a fixed column list, with `id` as a final tiebreaker so a tied sort never duplicates or drops a row across pages) rather than in the browser, which would silently only reorder the page on screen. Every other table in the app sorts client-side. `/rules` is the one deliberate exception, everywhere: its row order **is** its evaluation order (first match wins by priority), so it has no sortable headers — the existing move up/down controls are how its order changes.
...
Each leg of a transfer is categorized independently, and nothing before this confirmed the two legs actually correspond to each other — one mis-categorized leg silently inflates both spending and income by the same amount. The **Transfer Matching** card on `/transactions` (`services/transfer_matching.py`, `GET /api/transfers`) surfaces two things, and changes nothing on its own:

- **A candidate pair with mismatched categorization** — two transactions, in two different accounts, with exactly opposite amounts within a few days of each other, where at least one leg isn't categorized as a transfer. Matched by amount and date proximity only, deliberately **not** by narration: unlike a recurring merchant (whose narration is genuinely consistent occurrence to occurrence), a real bank transfer is usually worded differently by each side's own statement ("Transfer to XXXX1234" vs. "Transfer from XXXX5678"), so requiring narration agreement would miss the exact case this exists to catch.
- **An unmatched transfer leg** — a transaction categorized as a transfer with no matching counterpart found anywhere, meaning either the other side hasn't been imported yet, or this one isn't really a transfer.

A pair within the **same** account is never matched (a purchase and its refund, for instance, are not a transfer between accounts), and nothing here ever recategorizes a transaction automatically — the same "surface it, don't guess" posture as balance-sign inference (see [Accounts and net worth](#accounts-and-net-worth)).
...
### Rules v2 — OR patterns and regex

A rule's narration match is no longer limited to one plain substring. **Also match (OR)** on the rule form adds any number of extra patterns — the rule matches if the narration contains *any* of them — and **Treat patterns as regular expressions** switches every pattern (the primary one and every extra) from a substring check to a case-insensitive regular expression (`re.search`, unanchored). Both are opt-in and additive: a rule that uses neither behaves *exactly* as it always has, which is every rule created before this existed. `services/categorization.py`'s single matcher (`rule_patterns`/`narration_matches`) is still the only place any of this is evaluated, shared unchanged by import, preview and apply — see that module's own docstring for why one matcher for all three is load-bearing. An invalid regex is rejected with a 422 at save time (and by Check matches), naming the specific pattern that failed.
...
The containment check generalizes cleanly to a Rules v2 rule with several OR'd patterns: an earlier rule's pattern set covers a later one exactly when *every one* of the later rule's own alternatives contains at least one of the earlier rule's own alternatives — the direct extension of "the earlier pattern is a substring of the later one" from one pattern each to two sets of them, and the reason two genuinely unrelated single-substring patterns (`WOOLWORTHS`, `COLES`) still can't be merged into each other even now — the schema not being able to express that *is* Rules v2's OR feature, not a limitation review has to work around. "Merge" therefore still means *remove the rule that can never fire* for the common case. A **regex** rule (either side of the comparison) is excluded from this analysis entirely — whether one arbitrary regex's match set contains another's isn't decidable in general, so review makes no `duplicate`/`subsumed`/`shadowed` claim about a regex rule at all rather than risk an approximation being wrong in either direction. **Remove all duplicate/subsumed rules** (`POST /api/category-rules/review/remove-redundant`) deletes every `duplicate`/`subsumed` finding in one action and never touches a `shadowed` one; each finding can also be removed individually. Since both auto-removable kinds are provably no-ops, deleting them cannot change how a single transaction categorizes — re-running **Apply rules now** afterward recategorizes exactly the same rows.
...
### Budget rollover (sinking funds)

A category can opt in to **rollover** — unspent budget accumulates into next month's available amount, and overspend carries forward as a deficit. This is what makes an annual expense (car registration, insurance) work as a monthly budget: put $100/month toward a $1,200 bill and the six quiet months build up $600 of headroom, so the month the bill actually lands reads as comfortably under budget instead of a wild spike.

Turn it on with the **"Roll unspent budget into next month"** checkbox on a category's budget field (`/categories`). Accumulation starts from the month it was switched on — not the category's creation date — and stops (clearing everything accumulated) the moment it's switched off; turning it back on later starts a fresh accumulation rather than resurrecting the old one.

This is answered by a second, dedicated resolver, `services/budgets.rollover_available()`, deliberately kept separate from `effective_budget()` above: "what's my nominal budget for this month" and "what's actually available including everything carried forward" are different questions, and conflating them would mean every existing caller of `effective_budget()` would need to become rollover-aware just to keep answering the narrower one. Only `/reports` and the Monthly Budgets editor read the accumulated figure; a rollover category's `available_amount` (not its bare `budget_amount`) is what "over/under budget" is actually computed against in both places, and the Monthly Budgets table shows both figures side by side so the two never get confused with each other. `/trends`' multi-month grid still shows the nominal, non-accumulated figure for a rollover category — a known, deliberate scope limit (see `ROADMAP.md`'s [T2.1](ROADMAP.md#t21--budget-carryover-and-sinking-funds)).

### Pay period budgeting

The Queensland preset household is paid fortnightly, and three months a year hold three pay cycles — a strictly calendar-monthly budget mis-states those months in both directions. The **Pay Period Budgeting** card (`/categories`, below Monthly Budgets) is a *view* over the existing monthly budgets, not a second, independently-edited budget period: set one known payday (`PaySchedule.anchor_date`, a single household-wide row), and every category's standing monthly `budget_amount` is rescaled to a fortnightly pace (`amount * 12 / 26` — the real pay-calendar convention, not a more "precise" 365.25/14 that no fortnightly payslip actually follows) and compared against that fortnight's real spending.

`services/pay_periods.pay_period_bounds()` counts whole 14-day steps forward or backward from the anchor to find any date's fortnight, so it works identically before or long after the anchor and across a calendar-year boundary — nothing resets at January 1st or at a month edge. Pacing deliberately reads only the standing amount, never a monthly override: a fortnight can straddle two different calendar months, each potentially carrying its own override, and there's no principled way to pick which one a 14-day window should defer to. Nothing about how budgets are stored or reported elsewhere changes; a household that never sets a payday sees nothing different anywhere else in the app.
...
Import is the only way a transaction enters the ledger, so a forgotten statement period leaves a hole nothing else in the app would otherwise notice — every downstream total just reads as slightly lower than reality. `/accounts/:id`'s **Statement Coverage** card (`GET /api/accounts/{id}/coverage`, `services/coverage.py`) turns this into something *proven* rather than assumed, because every transaction already carries the bank's own running Balance: for two consecutive transactions on one account, the later one's balance must equal the earlier one's balance plus its own signed amount, which is arithmetic identity, not a heuristic. A break — reported with the date range on either side and the size of the discrepancy — is almost always a missing statement, though a duplicate or corrupted row produces the identical signature.

Two things the check deliberately does **not** do, both to avoid false positives rather than missed gaps:

- It never compares two transactions on the **same day** — a bank's own file order for one calendar date doesn't reliably become import id order, so the check only ever runs between the *last* row of one date and the *first* row of the next distinct date. A real missing-statement gap almost always spans whole days or weeks, so this costs nothing in detection power.
- It never crosses **accounts** — a [grouped account](#account-groups)'s members are genuinely different bank accounts in succession, and a replacement's opening balance has no arithmetic relationship to its predecessor's closing one. Each account is checked purely on its own transactions; group membership never enters this check at all.

A brand new account's very first transaction has nothing before it to compare against — that's the start of its recorded history, not a gap.
...
Every chart in the app is clickable, in whichever direction has more detail to give — `LineChart` reports what was clicked via `onSelectPoint`/`onSelectSeries`, `BarChart` via a per-bar `onSelectBar` (mirroring `LineChart`'s own per-point shape, not one hit area per whole column) — a chart with none of those props passed renders exactly as it always did, with nothing focusable and no pointer cursor. A series can opt out of an affordance that would otherwise do nothing (`selectable: false` — /trends' summed "Other" line, a projected Forecast series, "Net saved"/"Budgeted" in a comparison chart) rather than offer a dead click.

- **`/trends`' spending-by-category chart** shows one line per **group**, with a [grouped](#sub-categories) category's children rolled up into their parent rather than competing with their siblings for one of the six lines — a preset chart of accounts has enough leaves that the limit otherwise buries most of a household's spending in "Other" (`utils/trendsSeries.js` builds the levels). Clicking a group (a point on its line, or its name in the legend) drills into that group's own children, same chart. There is no third level, since categories are one level deep, so clicking a **leaf's** point opens the ledger filtered to that category and that month. The drilled-into group lives in the URL (`?group=`) next to `?months=`, so a drilled-in chart is reloadable and shareable; changing the window resets it, since a group with no activity in the new window isn't in the data at all.
- **Income vs spending charts** (`/trends`, and the Dashboard's own Cash Flow widget at a shorter window) drill by **kind**: the Income bar opens the ledger filtered to `kind=income` for that month, Spending to `kind=expense` — the ledger's own Category header filter gained an Income/Expenses/Transfers option to make `kind` a real, clearable filter there too, not just a URL param a chart can reach but the user can't. "Net saved" has no kind of its own (it's a difference, not a transaction) and isn't clickable.
- **Budget vs actual** (`/trends`) drills its Actual bar the same way, to `kind=expense` — the chart itself is already expense-scoped (see below), so that's the only honest target. "Budgeted" isn't clickable — no transaction is ever "budgeted".
- **A balance-history point** (`/accounts/:id`, and the Dashboard's Net Worth chart) opens the ledger for that month — scoped to the one account for `/accounts/:id`, unscoped for the Dashboard's own combined Net Worth chart, since net worth is a balance *snapshot*, not a sum of that month's transactions, so there's no kind or category to filter to.
- **Forecast is the deliberate exception** — every series there is a projection of months that mostly haven't happened yet, so there are no real transactions behind a point to open the ledger to.
...
`/` is a grid of **widgets** — Cash Flow and Net Worth still share the same months rather than combining them into one chart (cash flow is thousands, net worth tens of thousands; a shared axis would invent a correlation from an arbitrary alignment rather than show a real one), and the two gate independently, since a quiet month with no categorized activity can still show a balance trend, and vice versa — but which widgets appear, in what order, and how wide, is now the user's own choice rather than a fixed page.

### Getting Started

A fresh install has nothing to show a widget grid *from* - `/` shows a **Getting Started** checklist instead (`components/OnboardingChecklist.jsx`) until every step is done: import a statement, classify accounts (give each one a type), set up categories (the Queensland preset, or your own), and set at least one budget. Every step is an existing feature - this is sequencing and copy, not new machinery, and the component self-fetches just enough (`GET /accounts`, `GET /categories`, a 1-row `GET /transactions`) to know what's already done. It renders nothing once every step is complete - there's no dismiss state to persist, because "done" already is the dismissal condition - and appears as an ordinary card above the widget grid on an in-progress install, not only on a completely empty one.

**Layout is server-side state** (`GET`/`POST`/`PUT`/`DELETE /api/dashboard/widgets`, `backend/app/api/dashboard.py`, backed by `DashboardWidget` in `models.py`), reordered the identical way `/rules` already reorders itself — `POST .../move` swaps two widgets' `position` values, not a renumbering of the whole list. The table stores layout only, never data: every widget still reads live from the same endpoints the page always used, so a widget can never show a stale or duplicated figure. A new install's layout is **seeded by the migration that creates the table** (`e4a2c9f1b7d3_add_dashboard_widgets`), not by the application on first read of an empty table — a user removing a widget entirely has to stay removed, which an app-level "seed if empty" check can't tell apart from "nobody has customised this yet."

**Edit dashboard** turns on move-up/move-down, a width selector, and Remove per widget — real, keyboard- and screen-reader-operable buttons first, the same as `/rules`' own reordering, rather than drag-and-drop as the only mechanism. **Add new widget** opens a form driven entirely by `src/widgetRegistry.jsx` — the single source of truth for every `widget_type` (mirroring `pageRegistry.jsx`'s "one entry, wired in everywhere" idiom for pages), which fields it needs (a stat tile's metric, a calendar's month count, a sparkline's comparison basis), and its default width.

**One shared fetch per data source, not one per widget instance** is what keeps network traffic flat regardless of how many widgets are on the page — four stat tiles share one `GET /api/reports/kpis` call, exactly like Cash Flow and the Net Worth chart already shared one `GET /api/trends` call before widgets existed. The two exceptions are the widgets whose own window is genuinely per-instance config (a calendar showing last month vs this month, a sparkline comparing against different bases) — **Transaction Calendar** and **Spending Pace** fetch their own data independently.

The four new widget families:

- **Stat tiles** (`widget_type: stat_tile`) — one headline figure over a period: Total Income, Total Expenses, Avg Per Month, Avg Per Transaction, Net Saved, Savings Rate, Runway, or a plain transaction count, each reading `GET /api/reports/kpis` (`services/dashboard_metrics.dashboard_kpis`, built on `reporting.category_grid`/`trends.monthly_summaries` for its totals, not a second independent aggregation). Avg Per Month and Avg Per Transaction are both **spend-rate** figures — total expenses divided by the window's months, or by its count of expense transactions (counted `DISTINCT` by transaction, so a split grocery run counts once, not once per allocation) — deliberately not mixed with income, which would answer neither "how much do we spend a month" nor "how much do we earn". **Savings Rate** (`net_saved / total_income`) and **Runway** (`services.net_worth.liquid_assets() / avg_per_month` — current Everyday + Savings balances against this window's own average spend, not a projection; `services/forecast.py` already owns actual month-by-month projection) are both ratios, rendered as a percentage or a count of months (`utils/format.js`'s `formatPercent`/`formatMonths`) rather than a dollar figure — each `stat_tile` widget can pass its own `formatValue` to `StatTile` for exactly this reason. Both are **undefined, not zero**, on the one denominator that makes them meaningless: no income for the window (a household living off savings that month has no "rate" to report, not a rate of negative infinity), or no expenses to divide runway's liquid assets by. A tile whose metric maps to a real kind (income/expense) is clickable, opening the ledger filtered to that kind across the whole window; Net Saved, Savings Rate, Runway and the plain count aren't, since none of them is a transaction kind — there is no "the transactions behind a percentage" to open the ledger to. Rendered without `<Amount>`'s tabular numerals — a large standalone figure reads better in the font's normal proportional numerals, reserving tabular alignment for columns of numbers that must line up.
- **Net Worth Change** — the same delta idea as a stat tile, but sourced from `/trends`' own `balances` series (the identical data the Net Worth chart already fetches) rather than a new query: the change between the first and last *known* balance in the window (a gap before an account's history starts is skipped, not treated as zero).
- **Transaction Calendar** (`GET /api/reports/daily?date_from=&date_to=`, `services/dashboard_metrics.daily_activity`) — one month grid per `months` in its config, each day showing a bar sized by that day's net (in minus out) and colored by direction — green above the baseline, red below, the same success/danger pair `<Amount>`/`Badge` already use for a positive/negative figure everywhere else, not a sequential single-hue heatmap, because a day's activity genuinely has two poles (money in, money out), not one magnitude. A day with matching activity is clickable, opening the ledger filtered to just that day. `daily_activity` excludes transfers (matching every other report) but, unlike `uncategorized_summary`, does *not* separately exclude uncategorized transactions — it deliberately agrees with the existing Cash Flow chart's own scope (categorized, non-transfer activity), so a month's daily totals summed together equal that month's `total_income`/`total_spending` from `/trends`.
- **Spending Pace** (`comparison_sparkline`) — this month's cumulative spend against a comparison basis (last month, the average of the last 3 months, or a monthly budget total spread evenly across the days of the month), with the gap at the most recent day both curves have a value for as the headline figure. The comparison curve renders `muted: true` (see [Drilling down](#drilling-down) above) — it's context, not the point. Built from `utils/sparklineComparison.js`'s pure day-arithmetic (kept separate from the widget's own data-fetching for the same testability reason `utils/trendsSeries.js` is split from `TrendsPage.jsx`).

## Alerts

`/alerts` (also a badge on the nav link, e.g. "Alerts (3)") is one assembled feed over signals the app already computes elsewhere — **assembly, not a second implementation of any of them** (`services/alerts.py`):

| Kind | Sourced from |
|---|---|
| Over budget | `reporting.category_totals_for_period`/`budget_lines`, this month |
| Missed / stopped, price change | `recurring.detect_series` — an `overdue`/`ended` series, or one whose `amount_changed` |
| Coverage gap | `coverage.account_coverage_gaps`, per account ([Statement coverage](#statement-coverage)) |
| Unmatched transfer | `transfer_matching.unmatched_transfer_legs` ([Transfer matching](#transfer-matching)) |

Dismissing an alert never changes the underlying fact it's reporting on — only whether it's shown. A recurring-sourced alert (missed/stopped, price change) dismisses through the **existing** `POST /recurring/dismissals` — the identical action `/recurring`'s own Dismiss button already performs, keyed on `(account_id, narration_key)` — rather than a second, parallel dismissal system for something that already has one; `detect_series()`'s own default already excludes a dismissed series, so there's nothing extra to track. The other three kinds had no dismissal concept before this, so they share one new, generic table (`AlertDismissal`) keyed on a deterministic string built from the alert's own identity (e.g. `over_budget:{category_id}:{year}:{month}`) — never a surrogate row id, since there is no persisted "alert" to reference, only the recomputed fact. A *different* occurrence of the same kind of problem (a different month, a different account, a different transaction) gets a different key and is never silently suppressed by an old dismissal of something else.
...
The **Scenarios** card on `/forecast` (`POST /api/forecast/scenario`) answers "what if" without touching any real data — nothing is ever written to the database, and running the identical scenario twice always returns the identical answer. Two independent, combinable overlays:

- **Stop a recurring commitment** — a checkbox per upcoming series, hypothesising it ends today for this one projection only. This is deliberately **not** the same thing as dismissing it (`/recurring`'s own Dismiss): a scenario-stopped series is still exactly as recurring as ever for the everyday-run-rate exclusion above (it genuinely did happen historically) — only its *future* occurrences in this one hypothetical projection are skipped.
- **Adjust a category's everyday spending** — pick a category and a percent change (e.g. `-20`); `services/forecast.category_daily_rates()` decomposes the same run-rate window `daily_run_rates()` already computes, per category, so the adjustment is applied to the exact baseline figure rather than a second, independently-computed rate that could silently disagree with it. Only categorized, non-split, non-recurring activity can be targeted this way — a known, documented approximation (see that function's own docstring), not a second full reporting surface.

Both overlays fold into the same `project()` function every other caller already uses — there is no second, parallel projection algorithm. When a scenario is running, the chart swaps to a direct two-line comparison (baseline muted, scenario in front) rather than cluttering the default per-account view.
...
**The one structural rule**, and the whole reason this stack can share a NAS with other applications: **`api` joins the shared `dbnet`, `web` never does.** Docker's embedded DNS is per-network — a container resolves only the names of containers on networks *it* is attached to — so `web`, being on this project's own default network and nothing else, resolves `api` to exactly one thing: this project's own API. It does not matter how many other applications on the NAS also name a service `api`; none of them are on this project's network, so none of them are visible to resolve. `frontend/nginx.conf` therefore targets the plain service name `api:8000`, and needs no agreement with the compose file beyond that name.

Steps:

1. On the NAS, confirm the existing Postgres application's network is named `dbnet` and note the Postgres container's name (`docker network ls` / `docker inspect <postgres-container>`, or Container Station's network view). The Postgres user needs `GRANT CREATE ON SCHEMA public` so the app's startup migrations can create its tables — without it, the api container starts but every database-backed request fails, and the failure is otherwise silent (see `initialize_database()` in `backend/app/main.py`, which logs the error to stderr on failure — check the container logs if `database_ready` is ever `false`).
2. **Create the application with app name `homebudget`**: Container Station → Create > Application > pull from GitHub, pointing at `https://github.com/RapidChrisCircle/homeBudget` and `docker-compose.qnap.yml`. The application name becomes the Compose *project* name, which is what scopes the private network in the rule above — so it must be distinct from every other application on the NAS.
3. Container Station will detect the variables referenced in the compose file (it does not read `.env` files) and prompt for them:
   ...
   | `DATABASE_HOST` | container/service name of the existing Postgres container on `dbnet` |
   ...
   | `WEB_PORT` | host port to publish the web UI on (default `8080`) — **must be unique per application on this NAS** |

   `ALLOWED_ORIGINS` and `DEBUG_SQL` are not set by this compose file; the backend defaults them (`backend/app/database.py`) and neither matters for a QNAP deployment where the browser only ever talks to nginx. The API is deliberately **not** published on a host port for the same reason.

4. Deploy. The app is reachable at `http://<nas-ip>:8080` (or whichever `WEB_PORT` you chose).

Images are published as both `:latest` and `:<commit-sha>`. `docker-compose.qnap.yml` uses `:latest`.

> **Watchtower auto-updates and database migrations.** Both services carry `com.centurylinklabs.watchtower.enable=true`, so a new `:latest` image is pulled and the container recreated **unattended**. For `web` that is harmless. For `api` it is not entirely: `initialize_database()` runs Alembic migrations against the live database on every start, so a release containing a migration will alter your schema at whatever hour Watchtower happens to run, with no backup step and nobody watching. Keep a database backup, and when a release contains a migration you would rather apply deliberately, pin that service to a specific build — replace `:latest` with `:<commit-sha>` in Container Station's compose editor, from the repository's Actions/Packages history. Watchtower also updates the two images independently, so the frontend and API can be briefly out of step; the footer's version-mismatch banner (see [Versioning](#versioning)) is what surfaces that, and it resolves itself on the next update.

### Running several apps from this template on one NAS

Everything above is designed to make this safe, but four things are the deployer's responsibility:

- **A distinct Container Station application name per app.** It becomes the Compose project name, which names the private network that keeps each app's `api` unambiguous.
- **A distinct `WEB_PORT` per app.** Every clone of this template defaults to `8080`; the second one to start simply fails to bind.
- **Only `api` on `dbnet`, never `web`** — in every app. This is the rule that stops one app's nginx reaching another app's API. If you ever add `dbnet` to a `web` service to "make networking simpler", you have recreated the exact bug documented below.
- **No `container_name:`.** Container names are unique per Docker *host*, so a pinned name that two apps share means the second refuses to start. Leaving it out lets Compose derive a per-project name automatically.

One residual quirk worth knowing: a Compose service is always reachable by its own service name on every network it joins, and that alias cannot be removed without renaming the service. So this app's `api` does still offer the generic name `api` **on `dbnet`**. Nothing here resolves over `dbnet`, so it cannot affect this app — but a neighbouring application that (incorrectly) resolves `api` while attached to `dbnet` may reach this API. If that ever matters, rename the services in that app's own compose file rather than this one's.

### Troubleshooting: API returns 404 but the page loads

**Cause: the `web` container can see the shared `dbnet`, where another application's `api` answers too.** A Compose service is reachable by its own service name on every network it joins, so on `dbnet` *every* application's api service claims the generic name `api`. If this app's `web` is attached to `dbnet` (it must not be — see the structural rule above), the name `api` resolves to **several** containers and nginx round-robins across them. One request lands on this app's real API and works; the next, seconds later, lands on a different application entirely and 404s every path it doesn't recognise — which is *every* path, since it isn't homeBudget. That is why the failure moves around on reload instead of failing the same way each time, and why `GET /api/version` in the footer can report a correct build at the very moment another card fails: two different servers answered.

**The fix: take `web` off `dbnet`.** Only `api` needs it, and only to reach Postgres. With `web` on the project's own network alone, `api` resolves to exactly one container and the collision cannot occur. Confirm with `docker inspect` (or Container Station's network view) that the web container lists **one** network, and that it is not `dbnet`.
...
**Most likely cause with `Host not found`: `web` and `api` share no network.** If the api service declares `networks: [dbnet]` while `web` declares none (so it lands on the project default), the two are on entirely separate networks and `web` cannot resolve `api` by any name at all. The api service needs **both**: `networks: [dbnet, default]`. This is the mirror image of the 404 case above — there `web` had too much network access, here `api` has too little.

**Second cause with `Host not found`: the deployed compose names the service something other than `api`.** nginx targets `api:8000` by default. Compose names containers `<project>-<service>-<index>`, so the containers list tells you the service names directly: `homebudget-api-1` means the service is `api` (correct — project `homebudget`, service `api`, index 1), while `homebudget-homebudget-api-1` would mean the service is `homebudget-api` (a naming scheme an earlier version of this file briefly used and has since reverted; update the deployed YAML).

**`Connection refused` instead** means the name resolved but nothing is listening — the api container is up but its process isn't ready or has crashed. Check its logs; if `initialize_database()` failed, `database_ready` will be `false` in `GET /` on the API.

**If none of that applies**, `API_UPSTREAM` is the escape hatch — nginx reads it at container start, so pointing it at whatever name *does* resolve is an environment-variable change on the `web` service rather than a new image. It isn't set in the compose file (the default is correct for any stack built from this template), so add it under the web service's `environment:`. From a shell in the web container, find a name that works:

```
getent hosts api                                  # the configured default
getent hosts <the API container's exact name from Container Station's list>
```

Whichever resolves to exactly one address is a working value — set `API_UPSTREAM` to `<that name>:8000` and recreate the application (an environment change requires recreating, the same as any other compose edit). Confirm it is genuinely this app before relying on it: `wget -qO- http://<name>:8000/api/version` should report the current version and a commit that appears in this repository's `git log` — not a foreign build, per the 404 section above.
