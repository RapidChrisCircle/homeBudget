...
- **Where:** new `backend/app/services/coverage.py`, surfaced on `/accounts/:id` (`frontend/src/pages/AccountDetailPage.jsx`) and as a candidate signal for the alerts feed ([T2.4](#t24--alerts-feed)).
- **The arithmetic:** ordered by `(transaction_date, id)` within an account, consecutive rows must satisfy `balance[n] - balance[n-1] == debit[n] + credit[n]` (amounts are signed, debits negative — see `services/reporting.py`'s module docstring). A break is a gap in coverage, a duplicate, or an out-of-order import. Report the date range and the size of the discrepancy, which is usually enough to identify the missing statement.
- **Caveats:** same-day transactions can be exported in an order that doesn't match the bank's own running balance, so consecutive same-day rows need tolerance rather than a hard failure. An account whose first-ever transaction is mid-history has no opening balance to check against — that's the start of coverage, not a break. Grouped accounts (README's [Account groups](README.md#account-groups)) hand over between members and must not be treated as one continuous series.
- **Acceptance:** a seeded gap is detected with the right range and amount; a clean account reports no gaps; same-day ordering does not produce false positives; a grouped account's handover does not.
...
- **Where:** new export endpoints under `backend/app/api/`, reachable from the ledger toolbar and `/accounts`.
- **Notes:** export the **filtered** ledger, not just everything — the filter set is already expressed in the URL and `ledgerFilterParams.js` already translates it. Include split allocations and notes, or state plainly that the export is transaction-level only. A whole-database JSON dump is the backup case; filtered CSV is the "send it to the accountant" case, and they're different enough to be two things.
- **Acceptance:** a filtered export contains exactly the rows the ledger showed for that filter; a re-import of an exported CSV round-trips without loss (or the lossy fields are documented).
...
- **Where:** `backend/app/services/dashboard_metrics.py` (`dashboard_kpis` already computes income and expense totals for a window), `backend/app/services/net_worth.py` (liquid balances), `frontend/src/widgetRegistry.jsx` plus `components/widgets/StatTile.jsx`.
- **Definitions to settle and document:** savings rate = `(income − expenses) / income` over the window, undefined (not zero) when income is zero. Runway = liquid assets ÷ average monthly expenses, where **liquid** means Everyday and Savings account types — not Investment, not any liability. Both are ratios, so they need a percent/month suffix rather than `<Amount>`'s currency formatting.
- **Acceptance:** both tiles appear in the Add-widget list, read through the existing KPI fetch rather than adding a per-widget request (README's [Dashboard](README.md#dashboard) "one shared fetch per data source" rule), and a test pins the zero-income and zero-expense edge cases.
...
- **Where:** a new component mounted in `frontend/src/App.jsx`, reading `pageRegistry.jsx` for destinations.
- **Notes:** `⌘K`/`Ctrl+K`, searching pages, categories, accounts and merchants (`GET /transactions/groups` already produces merchant names). Must be fully keyboard-operable and dismissible on Escape — it is the one UI element that exists only for keyboard users. Debounce the transaction search; the page/category/account lists are already in memory.
- **Acceptance:** opens and closes by keyboard, arrow keys move the selection, Enter navigates, and results are grouped by kind.
